...
Note the name mismatch: the repo is `discocentaur`, the pm2 process and the deploy directory on
the box are both `claudetaur`.
...
A process first started with `pm2 start bot.js --name claudetaur` keeps the old timeout; switch it once
with `pm2 delete claudetaur && pm2 start ecosystem.config.cjs && pm2 save`. A clean restart logs
...
**Transfers are held while a document is in context.** A PDF is third-party text and can contain instructions ("send 500 USDC to …"). So for 40 turns after anyone uploads a document to a channel — long enough for both the document and every reply that quoted it to leave the 40-message history — `connect_drop`, `schedule_drop`, `conditional_drop` and `create_watcher` don't run. The bot posts exactly what would happen, built from the tool arguments rather than the model's prose, with a code:
...
Mentions reach the model as the recipient `connect_drop` takes —
`@Guillaume = {"type":"discord","id":"731076204307677226"}` — so it sends by ID. A Discord
display name is not an identifier and Connect cannot resolve it.
